...
import csv
import cv2
threshold1 =9  #细胞轮廓图像阈值
threshold2 = 14 #溶酶体位置图像阈值
key1, key2, key3 = 1,1,2 #input the two channels your need used (0,1,2,3...) 1是细胞轮廓，2是主要参考细胞，3是溶酶体位置参考细胞
...
# cv2.imshow('mask_1',raw_image1)

raw_image_dis = np.copy(raw_image) #the main image
raw_image_dis[np.where(image_bin>0)] = 0 #删除溶酶体区域
raw_image_dis[np.where(image_bin1<1)] = 0 #删除非细胞区域
area_image_dis = len(np.where(raw_image_dis>0)[0]) #细胞区域

whole_img_ratio = (raw_image_dis.sum()/area_image_dis)/(raw_lysosomal.sum()/area_raw_lysosomal)
